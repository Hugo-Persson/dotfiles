...
output=$(aichat "Please suggest 10 commit messages, given the following diff:
  ...
  **Instructions:**
  ...
  Write your 10 commit messages below in the format shown in Output Template section above." | fzf --height 40% --border --ansi --preview "echo {}" --preview-window=up:wrap)

# 2. Create a temporary file
tmpfile=$(mktemp /tmp/editmsg.XXXXXX)

# 3. Write the output to the temp file
echo "$output" >"$tmpfile"

# 4. Open the file in Vim for editing
vim "$tmpfile"
...
# 6. Do something with the edited content
